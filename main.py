...
class AndroidSAFBackend(object):
    ...
    def __init__(self):
        ...
        self._DocumentFile = None   # androidx, 延迟加载 (最可能出问题的一环)
        self._root_doc = None       # DocumentFile (tree)
        self._root_name = '(未授权)'

    @property
    def DocumentFile(self):
        # 延迟加载 androidx.documentfile: 即使它有问题, 也不会在后端构造时就崩,
        # 而是等到真正用时抛出可见的错误。
        if self._DocumentFile is None:
            self._DocumentFile = self.autoclass(
...
class BrowseScreen(Screen):
    def __init__(self, app, **kw):
        ...
        # 可点击横幅 (SAF 降级提示等), 默认隐藏
        self.banner = Button(text='', font_name=FONT, font_size=dp(12),
                             size_hint_y=None, height=0, opacity=0,
                             background_color=(0.7, 0.4, 0.2, 1))
        self._banner_cb = None
        self.banner.bind(on_release=lambda *_: self._banner_cb and self._banner_cb())
    ...
    def show_banner(self, text, on_tap=None):
        self.banner.text = text
        self._banner_cb = on_tap
        self.banner.height = dp(44)
...
class TextEditorApp(App):
    ...
    def _init_backend(self):
        if IS_ANDROID:
            # 优先 SAF; 若核心类都加载不了才降级 (androidx 已延迟加载, 不在此失败)
            try:
                self.backend = AndroidSAFBackend()
                self._saf_ok = True
            except Exception:
                self._saf_ok = False
                self._saf_error = traceback.format_exc()
                try:
                ...
                self.browse.show_banner(
                    'SAF 初始化失败, 已降级(读不到 Android/data)。点此看原因',
                    lambda: self._show_text_popup('SAF 失败原因', self._saf_error))
                return
    ...
    def _show_text_popup(self, title, text):
        box = BoxLayout(orientation='vertical', padding=dp(10))
        sv = ScrollView()
        lbl = Label(text=text or '(无)', font_name=FONT, font_size=dp(12),
                    halign='left', valign='top', size_hint_y=None)
        lbl.bind(width=lambda w, x: setattr(w, 'text_size', (x, None)),
                 texture_size=lambda w, s: setattr(w, 'height', s[1]))
        sv.add_widget(lbl)
        box.add_widget(sv)
        Popup(title=title, content=box, size_hint=(0.95, 0.8),
              title_font=FONT).open()
